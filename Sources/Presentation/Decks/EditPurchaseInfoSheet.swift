...
/// Sheet for editing the purchase metadata on a single PurchaseItem (store,
/// price paid, currency, notes, order URL) without changing its status.
///
/// Use this from places like the deck card detail where the user already
/// owns or ordered the card and just wants to record where they bought it
/// and what they paid. `MarkOrderedSheet` covers the related case of marking
/// a card as ordered for the first time.
struct EditPurchaseInfoSheet: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle("Edit Purchase Info")
    ...
    private func save() {
        let price = Double(pricePaidText
            ...
            .replacingOccurrences(of: ",", with: ""))
        // status is intentionally nil — this sheet only edits purchase metadata.
